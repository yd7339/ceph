...
extern "C" struct QZSTD_Session_S;

struct QZSTDSessionDeleter {
  void operator() (struct QZSTD_Session_S *session);
};
...
class QatZstd {
 ...
 public:
  ...
  using session_ptr = std::unique_ptr<struct QZSTD_Session_S, QZSTDSessionDeleter>;
  QatZstd();
  ~QatZstd();
 ...
 private:
  ...
  std::mutex mutex;
  std::string alg_name;
  struct QZSTD_Session_S *sequenceProducerState;
};
